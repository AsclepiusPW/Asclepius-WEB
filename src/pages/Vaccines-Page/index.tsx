...
export const VaccinesPage = () => {
    ...
    useEffect(() => {
        (() => {
            if (filter !== "") {
                switch (filter) {
                    case "withEvents":
                        setVisibleVaccines(allListVaccines?.filter(vaccine => vaccine.vaccinationCalendar.length > 0));
                        break;
                    case "recent":
                        setVisibleVaccines(allListVaccines?.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()));
                        break;
                    case "inEvents":
                        setVisibleVaccines(allListVaccines?.filter(vaccine => vaccine.vaccinationCalendar.length > 0));
                        break;
    ...
    const handleSearch = (query: string) => {
        if (query === "") {
            ...
            setVisibleVaccines(allListVaccines?.filter(vaccine => vaccine.name.toLowerCase().includes(query.toLowerCase())));
        }
    };

    return (
        <div className="container flex apresentation vaccineScreen" id="vaccineScreen">
            <Header
